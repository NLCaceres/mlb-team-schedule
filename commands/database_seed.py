...
from ..utility.datetime_helpers import (dateToday, isDatetime, dateToStr, strToDatetime, utcStrToPacificTime, 
...
def seedDB(updateMode = False):
    ...
    for dayNum, gameDate in enumerate(teamGameDates):
        ...
        currentSeriesGame = gameDate['games'][0]['seriesGameNumber']
        #* Next line checks for double headers BUT the above line just grabs the 1st of the 2 games (See next todo)
        gameStr = "Today's a double header! " if len(gameDate['games']) > 1  \
        ...
        seriesTotal = gameDate['games'][0]['gamesInSeries'] #todo Using the 1st game MIGHT cause next line to fail
...
def createTodaysGames(todaysGames, gamesInDb, seasonGameNum, gameStr): #* Make games for particular date
    ...
    for game in todaysGames: #* In case of double headers
        ...
        if gameNotInDB: #* If no record found, let's make one and set it!
            ...
            currentGameInDb = DodgerGame(date=strToDatetime(game['gameDate'], ISO_FORMAT), 
        ...
        if gameNotInDB or currentGameInDb.gameNumInSeries != game['seriesGameNumber'] or \
           currentGameInDb.gamesInSeries != game['gamesInSeries']:
              continue
        ...
        if currentGameInDb.readableDate != utcStrToPacificTime(game['gameDate']):
            print('No match! Update the game!')
            currentGameInDb.date = game['gameDate'] #* Date might not be the only prop that needs updating!
...
def createPromotionsForGame(promotions, thisGame):
    thisDaysGame = thisGame if not isDatetime(thisGame) else DodgerGame.query.filter_by(date=thisGame).first()
    print(f"Today's Game, {thisDaysGame}, has {len(promotions)} promotions")
    if thisDaysGame is None: 
    ...
    for promo in promotions:
        ...
        if thisDaysGame is not None and len(thisDaysGame.promos) > 0: #* THEN update the promotions list for this game
