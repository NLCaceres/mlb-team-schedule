from mlb_team_schedule.utility.endpoint_constants import (
    BASE_MLB_LOGO_URL, LATEST_GAME_URL, SCHEDULE_ENDPOINT,
)
...
def test_format_schedule_endpoint():
    #? If endpoint used w/out `format`, then string keeps placeholders
    assert SCHEDULE_ENDPOINT == ("https://statsapi.mlb.com/api/v1/schedule?lang=en&sportId=1"
                                 "&hydrate=team,game(promotions)&season={seasonYear}"
                                 "&startDate={startDate}&endDate={endDate}&teamId={teamId}"
                                 "&gameType=R&scheduleTypes=games")
    ...
    filled_schedule_endpoint = SCHEDULE_ENDPOINT \
        .format(seasonYear = 123, startDate = "Y-M-D", endDate = "y-m-d", teamId=123)
    assert filled_schedule_endpoint == ("https://statsapi.mlb.com/api/v1/schedule?lang=en"
                                        "&sportId=1&hydrate=team,game(promotions)&season=123"
                                        "&startDate=Y-M-D&endDate=y-m-d&teamId=123"
                                        "&gameType=R&scheduleTypes=games")
